...
void *process(void *arg) {
    ...
    for (; i < pFormatCtx->nb_streams; i++) {
        ...
        if (pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (window) {
                ANativeWindow_setBuffersGeometry(window, vedio->pCodecCtx->width,
                                                 vedio->pCodecCtx->height, WINDOW_FORMAT_RGBA_8888);
            }
...
Java_com_pf_avsynchronization_PfPlayer_play(JNIEnv *env, jobject instance, jstring path_) {
...
JNIEXPORT void JNICALL
Java_com_pf_avsynchronization_PfPlayer_display(JNIEnv *env, jobject instance, jobject surface) {
    if (window) {
        ANativeWindow_release(window);
        window = 0;
    }
    window = ANativeWindow_fromSurface(env, surface);
    if (vedio && vedio->pCodecCtx) {
        ANativeWindow_setBuffersGeometry(window, vedio->pCodecCtx->width, vedio->pCodecCtx->height,
                                         WINDOW_FORMAT_RGBA_8888);
    }
}

JNIEXPORT void JNICALL
Java_com_pf_avsynchronization_PfPlayer_release(JNIEnv *env, jobject instance) {
    if (isPlay) {
        isPlay = 0;
